    ...
    if(isset($_POST['ghazale_ds_submit_new_form'])) {
        if (ctype_alnum(trim($_POST['ghazale_ds_new_form_name']))) {
            ...
            if (count($tables)== 0) {
                ...
                $wpdb->query($sql_form);
                $sql_input = "CREATE TABLE " . $table_name . "_inputs (id INTEGER(10) UNSIGNED AUTO_INCREMENT, field_id INTEGER(10), input_order INTEGER(10), field_input VARCHAR(3000) COLLATE utf8_bin, PRIMARY KEY (id))";
...
/**
 * Alter previous input tables
 */

function ghazale_ds_alter_previous_input_tables(){
    //Alter input tables - add input_order column
    global $wpdb;
    $table_name_prefix = $wpdb->prefix . "ghazale_ds_";
    $input_tables= $wpdb->get_results("SHOW TABLES LIKE "."'" .$table_name_prefix."%_inputs'");
    $order = 0;
    foreach($input_tables as $input_table){
        foreach($input_table as $table){
            $column_exists = $wpdb-> query("SHOW COLUMNS FROM `".$table."` LIKE 'input_order'");
            if(!$column_exists){
                $add_column = "ALTER TABLE ". $table. " ADD input_order INT(10) NOT NULL AFTER field_id";
                $wpdb->query($add_column);
                $last_field_id = "SELECT MAX(field_id) FROM ".$table. " LIMIT 1";
                $max_field_id_query = $wpdb-> get_var($last_field_id);
                $inputs_field_id = "SELECT id,field_id,input_order FROM ".$table. " ORDER BY id ASC";
                $inputs_field_id_query = $wpdb->get_results($inputs_field_id,ARRAY_A);
                foreach($inputs_field_id_query as $input){
                    if( $input['field_id'] <= $max_field_id_query ){
                        $wpdb->query('UPDATE '. $table.' SET input_order='.($order+1).' WHERE id='.$input['id']);
                    }
                    if($input['field_id'] == $max_field_id_query){
                        $order = $wpdb->get_var('SELECT MAX(input_order) FROM '.$table.' LIMIT 1');
                    }
                }
...
add_action('init','ghazale_ds_alter_previous_input_tables');
...
function ghazale_ds_add_fields(){
    ...
    <form action="" method="post" id="ghazale_ds_add_fields">
        ...
        <?php $type_array = array("Text Field","Text Area","Drop Down","Multiple Choice","Email","File Upload","Number","Telephone","URL","Date","Time","CheckBox","Instruction Text"); ?>
...
// Localize the script with new data
    ...
    wp_localize_script( 'ds-field-ext', 'ds_object_name1', $translation_array );
}
...
function ghazale_ds_register_settings(){
    ...
    register_setting('ghazale_ds_settings','ghazale_ds_confirmation_subject');
...
function ghazale_ds_admin_form_table_tabs_register(){
    ...
    $translation_array = array(
        'refresh3' => __( 'Refresh the page to restore "Update"/"Delete" links', 'data-storage' )
    );
    wp_localize_script( 'ds-tabs-script', 'ds_object_name2', $translation_array );
}
                ...
                if ((str_replace(array($table_name, '_fields'), '', $fields_table)) == $atts['form']) {
                    ...
                    foreach ($fields as $field) {
                        if($field['field_required'] == "Required"){
                            ...
                            if($field['field_type'] == "Instruction Text" || $field['field_type'] == "CheckBox"){
                                // don't put colon if the field type is Intruction Text or CheckBox
                            }else{
                                $output .= ":";
                            }
                            $required = "";
                        ...
                        switch ($field['field_type']){
                            ...
                            case "Date":
                                ...
                                break;
                            case "Time":
                                $output .= "<br><input type=\"time\" name=\"".$field['id']."\" size=\"30\" value=\"". (isset($_POST[$field['id']])? $_POST[$field['id']] : '') ."\" ". $required ." />" . "<br><br>";
                                break;
                            case "CheckBox":
            ...
            foreach ($table as $fields_table) {
                if (isset($_POST['submit_' . $fields_table])) {
                    if (trim($_POST['ds-captcha']) == ghazale_ds_captcha()) {
                        ...
                        $last_field_id = "SELECT MAX(field_id) FROM ".$input_table. " LIMIT 1";
                        $max_field_id_query = $wpdb-> get_var($last_field_id);
                        $determine_field_id = "SELECT field_id FROM ". $input_table;
                        $determine_field_id_query = $wpdb->get_results($determine_field_id,ARRAY_A);
                        foreach($determine_field_id_query as $check_field_id) {
                            if ($check_field_id['field_id'] == $max_field_id_query) {
                                $order = $wpdb->get_var('SELECT MAX(input_order) FROM ' . $input_table . ' LIMIT 1');
                            }
                        }
                        foreach($fields as $field){
                        ...
                        if (empty($empty_required) && empty($not_alphanumeric)) {
                            ...
                            foreach ($fields as $field) {
                                ...
                                $wpdb->insert($input_table, array('field_id' => $field['id'],'input_order'=> ($order+1), 'field_input' => (isset($_POST[$field['id']])? $_POST[$field['id']]: '')), array('%s'));
                            ...
                            if(get_option('ghazale_ds_confirmation_email')){
                                $to = $user_email;
                                if(get_option('ghazale_ds_confirmation_subject')){
                                    $subject = get_option('ghazale_ds_confirmation_subject');
                                }else {
                            ...
                            if(get_option('ghazale_ds_update_admin')){
                                ...
                                $header = 'From:' . get_bloginfo('name') .' <';
                                $header .= 'wordpress@'. get_bloginfo('name').'.com';
                                $header .= '>' . "\r\n" . 'Content-Type: text/html; charset=UTF-8' . "\r\n";
                                wp_mail($to_admin,$subject_admin,$message_admin, $header);
    ...
    if(!empty($input_tables)) {
        ...
        foreach ($input_tables as $input_table) {
            foreach ($input_table as $table) {
                ...
                echo "<h4 class=\"ds_refresh_page\" style=\"color: #f6a828\"></h4>";
                echo "<table class = \"table\" id=\"ds_download_table_csv_".$table."\">";
                $inputs_sql = "SELECT * FROM " . $table. " ORDER BY input_order DESC, id ASC";
                ...
                foreach($field_tables as $field_table){
                    foreach($field_table as $field_table_single){
                        if($field_table_single == str_replace("_inputs","_fields","$table")){
                            $fields_sql = "SELECT id,field_name,field_type FROM ". $field_table_single." ORDER BY id ASC";
                            ...
                            if(!empty($inputs)){
                                ...
                                echo "<th><span class=\"ds_del_link\">". __('Delete', 'data-storage'). "</span></th>";
                                echo "</tr></thead>";
                                foreach($fields as $field){
                                    $first_id = $field['id'];
                                    break;
                                ...
                                foreach($inputs as $key => $input){
                                    ...
                                    if(!in_array($input['field_id'], $instructions_field_id)) {
                                        foreach ($input as $key => $input_single) {
                                            ...
                                            if ($input_single != "" && $key != 'id' && $key != 'field_id' && $key != 'input_order') {
                                                if (substr($input_single, 0, 4) === "http") {
                                                    echo "<td><a href='" . $input_single . "' target='_blank'>" . $input_single . "</a><span class=\"ds_update_link\"><a href=\"".get_admin_url()."admin.php?page=ds-update-single-entry&update-single-entry-id=".$input['id']."&ds-entry-table=".$table."\" style=\"color: #e78f08\">(".__('Update','data-storage').")</a></span></td>";
                                                } else {
                                                    echo "<td>" . stripslashes($input_single) . "<span class=\"ds_update_link\"><a href=\"".get_admin_url()."admin.php?page=ds-update-single-entry&update-single-entry-id=".$input['id']."&ds-entry-table=".$table."\" style=\"color: #e78f08\">(".__('Update','custom-searchable-data-entry-system').")</a></span></td>";
                                                }
                                            } elseif ($input_single == "") {
                                                echo "<td><span class=\"ds_update_link\"><a href=\"".get_admin_url()."admin.php?page=ds-update-single-entry&update-single-entry-id=".$input['id']."&ds-entry-table=".$table."\" style=\"color: #e78f08\">(".__('Update','custom-searchable-data-entry-system').")</a></span></td>";
                                            }
                                        }
                                    }
                                    if($input['field_id'] == $last_id) {
                                        $last_input_id = $input['id'];
                                        echo "<td><span class=\"ds_del_link\"><a href=\"".get_admin_url()."admin.php?page=ds-form-entries&ds-del-entry-first-entry-id=".$first_input_id."&ds-del-entry-last-entry-id=".$last_input_id."&ds-del-entry-table-row=".$table."#".$table."\" onclick=\"return confirm('".__('Are you sure? (THIS ACTION CANNOT BE UNDONE)','data-storage')."');\" style=\"color: #ff0000\">".__('Delete Row','data-storage')."</a></span></td>";
                                        echo "</tr>";
                                    }
                                }
                                echo "</tbody>";
                                echo '<p><strong>'.esc_html__('Current number of entries in this table: ','data-storage').'</strong> '. $input_rows.'</p>';
                            }else{
    ...
    foreach ($input_tables as $input_table) {
        foreach ($input_table as $table) {
            ...
            if (str_replace(array($tables, '_inputs'), '', $table) == $atts['input']) {
                $output = "<div id=\"" . $table . "\">";
                $output .= "<table class = \"table\" id=\"ds_table_" . $table . "\">";
                $inputs_sql = "SELECT * FROM " . $table . " ORDER BY input_order DESC, id ASC";
                $inputs = $wpdb->get_results($inputs_sql, ARRAY_A);
                        ...
                        if ($field_table_single == str_replace("_inputs", "_fields", "$table")) {
                            ...
                            if (!empty($inputs)) {
                                ...
                                foreach ($inputs as $input) {
                                    ...
                                    if (!in_array($input['field_id'], $instructions_field_id)) {
                                        foreach ($input as $key => $input_single) {
                                            if ($input_single != "" && $key != 'id' && $key != 'input_order' && $key != 'field_id') {
                                                if (substr($input_single, 0, 4) === "http") {
    ...
    <form action="options.php" id="ghazale_ds_general_dettings" method="post">
        ...
        <textarea rows="4" cols="40" name="ghazale_ds_custom_captcha_msg"><?php
            ...
            <div>
                ...
                <?php _e('From','data-storage'); ?>: <input type="email" size="40" name="ghazale_ds_confirmation_from" value="<?php
                if(get_option('ghazale_ds_confirmation_from')) {
                ...
                <?php _e('Email Subject','data-storage') ?>: <input type="text" size="40" name="ghazale_ds_confirmation_subject" value="<?php
                if(get_option('ghazale_ds_confirmation_subject')) {
                    echo get_option('ghazale_ds_confirmation_subject');
                }else{
                    echo 'Your Recent submission on '.get_bloginfo('name');
                }
                ?>" /><i> <?php _e('Edit the subject of confirmation email','data-storage'); ?></i><br><br>
